...
function getSoundCloudId(track) {
  ...
  request.open(
    "GET",
    track + "?client_id=f240950ceb38d793cf52508943c8dc3f",
    true
  );
...
function getAudioList(val, init) {
  $.getJSON("https://api.soundcloud.com/tracks?q=hello&ids=1%2C2%2C3&urns=soundcloud%3Atracks%3A1%2Csoundcloud%3Atracks%3A2%2Csoundcloud%3Atracks%3A3&genres=Pop%2CHouse&tags=test&bpm%5Bfrom%5D=123&bpm%5Bto%5D=456&duration%5Bfrom%5D=123456&duration%5Bto%5D=456789&created_at%5Bfrom%5D=2020-12-24%2000%3A00%3A00&created_at%5Bto%5D=2020-12-26%2000%3A00%3A00&access=playable%2Cpreview&limit=2&offset=0&linked_partitioning=true").done(function (sounds) {
    ...
    if (init !== false) {
      setTimeout(function () {
        getSoundCloudId("//api.soundcloud.com/tracks/59581315/stream");
      }, 1000);
    }
  });
}
...
function preLoader(request) {
  request.addEventListener(
    ...
    function (event) {
      ...
      document
        .getElementById("preload-in")
        .setAttribute("style", "width:" + preload + "%;");
...
fons.onchange = function () {
  body.setAttribute(
    "style",
    "background-color:rgba(0, 0, 0, 0." + fons.value + ")"
  );
};
    ...
    if (hue != 0 || saturation != 0 || lightness != 0) {
      ...
      if (l > 0) {
        if (vm > 0) {
          ...
          if (r == vs) {
            if (g == ms) h = 5 + (vs - b) / vm + hue6;
            else h = 1 - (vs - g) / vm + hue6;
          } else if (g == vs) {
            if (b == ms) h = 1 + (vs - r) / vm + hue6;
            else h = 3 - (vs - b) / vm + hue6;
          } else {
            if (r == ms) h = 3 + (vs - g) / vm + hue6;
            else h = 5 - (vs - r) / vm + hue6;
          }
          if (h < 0) h += 6;
          if (h >= 6) h -= 6;
          var m = l + l - v;
      ...
      if (r < 0) d[i] = 0;
      else if (r > 255) d[i] = 255;
      else d[i] = r;

      if (g < 0) d[i + 1] = 0;
      else if (g > 255) d[i + 1] = 255;
      else d[i + 1] = g;

      if (b < 0) d[i + 2] = 0;
      else if (b > 255) d[i + 2] = 255;
      else d[i + 2] = b;
    }
  }
  return pixels;
